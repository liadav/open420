...
class QVAPDevice:
    def __init__(self, address, service_uuid, service_uuid1, characteristic_uuid, characteristic_uuid_serial):
        ...
        self.ble_process_pending = False
        self.timer_ble_pending_handler = None
    ...
    def set_ble_process_pending(self, status):
        self.ble_process_pending = status
        if status:
            if self.timer_ble_pending_handler is None:
                self.timer_ble_pending_handler = asyncio.get_event_loop().call_later(2, self.my_timer_ble_pending_timeout_func)
        else:
            if self.timer_ble_pending_handler:
                self.timer_ble_pending_handler.cancel()
                self.timer_ble_pending_handler = None

    def is_ble_process_pending(self):
        return self.ble_process_pending

    def my_timer_ble_pending_timeout_func(self):
        print("BLE operation timed out")
        self.set_ble_process_pending(False)
    ...
    async def connect(self):
        async with BleakClient(self.address) as client:
            ...
            # Send command to initialize and retrieve firmware status
            ...
            #if self.application_firmware == 0:
            #    print("Failed to set application firmware. Exiting.")
            #    return
    ...
    async def init_connection(self):
        if not self.is_ble_process_pending():
            self.set_ble_process_pending(True)
    ...
    async def final_stage(self):
        buffer = bytearray(20)
        buffer[1] = 6
        print(f"application_firmware={self.application_firmware}")
    ...
    async def parse_response(self, response):
        ...
        try:
            if command == 2:  # Command that updates the application firmware status
                ...
                self.application_firmware = APPLICATION_FIRMWARE_MASK_APPLICATION
