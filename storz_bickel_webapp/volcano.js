...
function eventsVolcano(state) {
    if (state == true) {
        ...
        vibrationToggleVolcano.addEventListener("click", vibrationToggleVolcanoHandler);
        displayOnCoolingToggleVolcano.addEventListener("click",
        ...
        vibrationToggleVolcano.removeEventListener("click", vibrationToggleVolcanoHandler);
        displayOnCoolingToggleVolcano.removeEventListener("click", displayOnCoolingToggleVolcanoHandler);
        try {
            characteristicWriteTempV.removeEventListener("characteristicvaluechanged", handleTargetTemperatureChanged);
            characteristicCurrTempV.removeEventListener("characteristicvaluechanged",
                handleCurrTemperatureChangedVolcano);
            characteristicPrj1V.removeEventListener("characteristicvaluechanged", handlePrj1ChangedVolcano);
            characteristicPrj2V.removeEventListener("characteristicvaluechanged", handlePrj2ChangedVolcano)
        } catch (e) {
...
function startAnalysisVolcanoFunc() {
    if (!isBleProcesPendingWithoutAlert_Repeat()) {
        ...
        characteristicPrj1V.readValue().then(value => {
            ...
            characteristicPrj3Value =
                convertBLEtoUint16(value);
            ...
            if (characteristicPrj1Value & MASK_PRJSTAT1_VOLCANO_ERR || characteristicPrj2Value & MASK_PRJSTAT2_VOLCANO_ERR) {
                let str = "Please contact STORZ & BICKEL with the following information: \n\n" + "SN   :   " + getSerialNumber() + "\n" + "date : 0x" + numHex(Math.floor(Date.now() /
                    1E3), 8) + "\n" + "hist1:   " + hist1 + "\n" + "hist2:   " + hist2 + "\n";
                alert(str)
            } else {
                var str = "";
                if (characteristicPrj2Value & MASK_PRJSTAT2_VOLCANO_DISPLAY_ON_COOLING) str = str + "- Temperature display during cool down phase disabled. Activate function if required.\n";
                if (characteristicPrj3Value & MASK_PRJSTAT3_VOLCANO_VIBRATION) str = str + "- Vibration when setpoint reached disabled. Activate function if required.\n";
                if (ledBrightness < 25) str = str + "- Display brightness set to low value. Change brightness if required.\n";
                if (str.length >
                    1) alert(str);
...
function handleTimeoutBLEResponseMissingHybrid() {
    console.log("handleTimeoutBLEResponseMissingHybrid repeat: " + repeatPageDataV);
...
function writePageDataSequenceHybrid() {
    if (!isBleProcesPendingWithoutAlert_Repeat()) {
        console.log(" writePageDataSequenceHybrid dataIdxHybrid: " + dataIdxHybrid + " page: " + pageIdxHybrid);
        preventScreenDimming(true);
        let data = binaryContentV.slice(dataIdxHybrid * 128 + pageIdxHybrid * 2048, (dataIdxHybrid + 1) * 128 + pageIdxHybrid * 2048);
        setBleProcessPending(true);
        ...
        let dataBuf =
            sendPageDataHybrid(data, dataIdxHybrid);
        var indexVar = 0;
        characteristicBootloaderWrite.writeValueWithoutResponse(dataBuf.slice(indexVar, indexVar + 20)).then(value => {
            indexVar = indexVar + 20;
            return characteristicBootloaderWrite.writeValueWithoutResponse(dataBuf.slice(indexVar, indexVar + 20))
        }).then(value => {
            indexVar = indexVar + 20;
            return characteristicBootloaderWrite.writeValueWithoutResponse(dataBuf.slice(indexVar, indexVar + 20))
        }).then(value => {
            indexVar = indexVar + 20;
            return characteristicBootloaderWrite.writeValueWithoutResponse(dataBuf.slice(indexVar,
                indexVar + 20))
        }).then(value => {
            indexVar = indexVar + 20;
            return characteristicBootloaderWrite.writeValueWithoutResponse(dataBuf.slice(indexVar, indexVar + 20))
            ...
            indexVar = indexVar + 20;
            return characteristicBootloaderWrite.writeValueWithoutResponse(dataBuf.slice(indexVar, indexVar + 6))
        }).then(value => {
            dataIdxHybrid = dataIdxHybrid + 1;
            timeoutBLEresponseHybrid = setTimeout(handleTimeoutBLEResponseMissingHybrid, 1500);
            if (dataIdxHybrid < 16) fsmHybrid = fsmNameHybrid.writePageData;
            else if (pageIdxHybrid < binaryNumberOfPagesV) {
                fsmHybrid =
                    fsmNameHybrid.flashPage;
    ...
    if (calcCheckValue(uint8Buf) == uint8Buf[len - 2]) {
        ...
        if (strChar.startsWith("RV0 222 BL")) {
            if (!isBleProcesPending()) {
                setBleProcessPending(true);
                characteristicBootloaderWrite.writeValue(getPageNumber()).then(value => {
                    setBleProcessPending(false)
                })
            }
            ...
            if (!isBleProcesPending()) {
                setBleProcessPending(true);
                characteristicBootloaderWrite.writeValue(getPageSize()).then(value => {
                    setBleProcessPending(false)
                })
            }
            ...
            if (strChar.includes(firmwareChecksumNew)) characteristicBootloaderWrite.writeValue(sendCRChecksum(firmwareChecksumNew)).then(value => {
                setBleProcessPending(false)
            });
            else if (strChar.includes(firmwareChecksumOld)) characteristicBootloaderWrite.writeValue(sendCRChecksum(firmwareChecksumOld)).then(value => {
                setBleProcessPending(false)
            });
            else {
                setBleProcessPending(false);
            ...
            if (fsmHybrid == fsmNameHybrid.writePageAddr) {
                ...
                if (!isBleProcesPending()) {
                    ...
                    if (pageIdxHybrid < binaryNumberOfPagesV) {
                        ...
                        repeatPageDataV = 0;
                        characteristicBootloaderWrite.writeValue(writePage(pageIdxHybrid)).then(value => {
                            setBleProcessPending(false)
                        })
                    } else {
                        fsmHybrid = fsmNameHybrid.idle;
                        characteristicBootloaderWrite.writeValue(getCRCChecksum()).then(value => {
                            setBleProcessPending(false)
                        })
                    }
                }
                ...
                setBleProcessPending(true);
                fsmHybrid = fsmNameHybrid.idle;
                characteristicBootloaderWrite.writeValue(exitBootMode()).then(value => {
                    setBleProcessPending(false);
                    alert("Firmware update successful: please reconnect to VOLCANO HYBRID.")
                })
            } else {
...
function startUpdateFuncHybrid() {
    ...
    if (whichDeviceConnected() == "Hybrid")
        if (!isBleProcesPending()) {
            fsmHybrid = fsmNameHybrid.idle;
            $("#firmwareUpdateModal").modal("hide");
            $("#device").modal("hide");
            ...
            return fetch("firmwareHybrid", {
                ...
                setTimeout(function() {
                    setBleProcessPending(true);
                    characteristicBootloaderWrite.writeValue(getBootStatus()).then(value => {
                        setBleProcessPending(false)
                    })
                }, 1E3)
            }).catch(error => {
...
function handleUARTBootloader(event) {
    tempBufUARTBootloader = _appendBuffer(tempBufUARTBootloader, event.target.value.buffer);
...
function volcanoConnect() {
    ...
    bluetoothDevice.gatt.connect().then(server => {
        ...
        return primaryServiceUuidVolcano1.getCharacteristic("00000003-1989-0108-1234-123456789abc")
    }).then(characteristic => {
        ...
        return primaryServiceUuidVolcano1.getCharacteristic("00000002-1989-0108-1234-123456789abc")
    }).then(characteristic => {
        characteristicBootloaderWrite = characteristic;
        return primaryServiceUuidVolcano3.getCharacteristic("10100011-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        codeNumberCharacteristic = characteristic;
        currentThenValue = 7;
        return primaryServiceUuidVolcano3.getCharacteristic("1010000d-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        currentThenValue =
            8;
        ...
        if (characteristicPrj2Value & MASK_PRJSTAT2_VOLCANO_ERR) deviceHasErrorsV = true;
        currentThenValue = 9;
        return characteristicPrj2V.startNotifications().then(_ => {
            characteristicPrj2V.addEventListener("characteristicvaluechanged", handlePrj2ChangedVolcano)
        })
    }).then(_ => {
        return characteristicPrj2V.readValue()
    }).then(value => {
        aside.classList.add("d-none");
        return primaryServiceUuidVolcano4.getCharacteristic("10110003-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicWriteTempV = characteristic;
        currentThenValue = 10;
        return characteristicWriteTempV.startNotifications().then(_ => {
            characteristicWriteTempV.addEventListener("characteristicvaluechanged", handleTargetTemperatureChanged)
        })
    }).then(_ => {
        ...
        return primaryServiceUuidVolcano4.getCharacteristic("10110001-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicCurrTempV = characteristic;
        currentThenValue = 11;
        return characteristicCurrTempV.startNotifications().then(_ => {
            characteristicCurrTempV.addEventListener("characteristicvaluechanged", handleCurrTemperatureChangedVolcano)
        })
    }).then(_ => {
        ...
        if (currentTemperature <
            6536) showCurrentTemperature(currentTemperature);
        currentThenValue = 12;
        return primaryServiceUuidVolcano4.getCharacteristic("1011000f-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicHeaterOnV = characteristic
    }).then(service => {
        currentThenValue = 13;
        return primaryServiceUuidVolcano4.getCharacteristic("10110010-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicHeaterOffV = characteristic
    }).then(service => {
        return primaryServiceUuidVolcano4.getCharacteristic("10110014-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        currentThenValue = 14;
        characteristicIsPumpOffV = characteristic
    }).then(service => {
        return primaryServiceUuidVolcano4.getCharacteristic("10110013-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicVolcanoPumpOn = characteristic
    }).then(service => {
        currentThenValue = 15;
        return primaryServiceUuidVolcano3.getCharacteristic("10100008-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        return characteristic.readValue()
    }).then(value => {
        let decoder = new TextDecoder("utf-8");
        setSerialNumber(decoder.decode(value).substring(0,
            8))
    }).then(service => {
        currentThenValue = 16;
        return primaryServiceUuidVolcano3.getCharacteristic("10100005-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        firmwareVersionCharacteristic = characteristic;
        if (typeof characteristicBootloaderWrite.writeValueWithoutResponse === "function") browserSupportsWriteWithoutResponse = true;
        ...
        return primaryServiceUuidVolcano3.getCharacteristic("10100003-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        currentThenValue = 17;
        return characteristic.readValue()
    }).then(value => {
        ...
        firmwareLabel.innerText = firmwareV.substring(0,
            8);
        var regex = /(\d+)\.?(\d+)\.?(\d+)\.?(\d+)$/;
        var result = regex.exec(firmwareV);
        if (result.length < 3 || firmwareFileMajorHybrid != result[1] || firmwareFileMinorHybrid <= result[2] || browserSupportsWriteWithoutResponse == false) {
            ...
            element.innerHTML = "The firmware of your VOLCANO HYBRID is outdated. The update will restart your VOLCANO HYBRID: please reconnect to finish the firmware update." +
        ...
        currentThenValue = 18;
        return primaryServiceUuidVolcano3.getCharacteristic("10100001-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        return characteristic.readValue()
    }).then(value => {
        let decoder = new TextDecoder("utf-8");
        ...
        if (bootloaderV.includes("BL"))
            if (browserSupportsWriteWithoutResponse == false) {
                alert("Your VOLCANO HYBRID is in firmware update mode: but your browser does not support the firmware update. Please use a compatible browser to finish the firmware update. We recommend using Bluefy for iOS or Chrome for Android, Mac and Windows.");
                firmwareUpdateLiItem.classList.add("d-none");
                startUpdate.disabled = true
            } else {
                var element = document.getElementById("firmwareUpdateText");
                element.innerHTML = "Your VOLCANO HYBRID is in firmware update mode: please finish the firmware update.";
                firmwareUpdateLiItem.classList.remove("d-none");
                startUpdate.disabled = false;
        ...
        return primaryServiceUuidVolcano3.getCharacteristic("10100004-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        return characteristic.readValue()
    }).then(value => {
        currentThenValue =
        ...
        return primaryServiceUuidVolcano4.getCharacteristic("1011000c-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicCurrentAutoOffTimeVolcano = characteristic;
        currentThenValue = 21;
        return characteristic.startNotifications().then(_ => {
            characteristic.addEventListener("characteristicvaluechanged", handleCurrentAutoOffTimeVolcano)
        })
    }).then(_ => {
        return characteristicCurrentAutoOffTimeVolcano.readValue()
    }).then(value => {
        autoOffTimeVolcano = convertBLEtoUint16(value);
        return primaryServiceUuidVolcano4.getCharacteristic("10110015-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicHeatHoursChangedVolcano = characteristic;
        currentThenValue = 22;
        return characteristic.startNotifications().then(_ => {
            characteristic.addEventListener("characteristicvaluechanged", handleHeatHoursChangedVolcano)
        })
    }).then(_ => {
        return characteristicHeatHoursChangedVolcano.readValue()
    }).then(value => {
        useHoursHeatVolcano = convertBLEtoUint16(value);
        return primaryServiceUuidVolcano4.getCharacteristic("10110016-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicHeatMinutesChangedVolcano = characteristic;
        currentThenValue = 23;
        return characteristic.startNotifications().then(_ => {
            characteristic.addEventListener("characteristicvaluechanged", handleHeatMinutesChangedVolcano)
        })
    }).then(_ => {
        ...
        updateHoursLabel();
        return primaryServiceUuidVolcano3.getCharacteristic("1010000c-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicPrj1V = characteristic;
        currentThenValue = 24;
        return characteristicPrj1V.startNotifications().then(_ => {
            characteristicPrj1V.addEventListener("characteristicvaluechanged", handlePrj1ChangedVolcano)
        })
    }).then(_ => {
        ...
        if (characteristicPrj1Value & MASK_PRJSTAT1_VOLCANO_ERR) deviceHasErrorsV = true
    }).then(service => {
        currentThenValue = 25;
        return primaryServiceUuidVolcano3.getCharacteristic("1010000e-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicPrj3V = characteristic;
        return characteristic.readValue()
    }).then(value => {
        let currentVal = convertBLEtoUint16(value);
        if ((currentVal & MASK_PRJSTAT3_VOLCANO_VIBRATION) == 0) vibrationToggleVolcano.checked = true;
        else vibrationToggleVolcano.checked = false;
        currentThenValue = 26
    }).then(service => {
        return primaryServiceUuidVolcano3.getCharacteristic("10100015-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicHist1 = characteristic;
        return primaryServiceUuidVolcano3.getCharacteristic("10100016-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        characteristicHist2 = characteristic;
        currentThenValue = 27;
        return primaryServiceUuidVolcano4.getCharacteristic("1011000d-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        ...
        return primaryServiceUuidVolcano4.getCharacteristic("10110005-5354-4f52-5a26-4249434b454c")
    }).then(characteristic => {
        ...
        if (error.toString().includes("User cancelled")) $("#waitModal").modal("hide");
            ...
            $("#waitModal").modal("hide");
            alert("Bluetooth connection error HYBRID: please reload and retry.\n" +
                error.toString() + "\n" + error.stack + "\n" + currentThenValue)
        }
    })
}
...
function onPlusButtonMouseDownVolcano() {
    if (whichDeviceConnected() == "Hybrid") {
        ...
        timerPlusButtonMouseDown = easingTimeout(1E3, function() {
            if (myTimerSetTempHandler !== 0) window.clearTimeout(myTimerSetTempHandler);
            myTimerSetTempHandler = window.setTimeout(myTimerSet,
                timeInMilliSeconds);
            if (isCelsiusActive() == true) setSollTemperatureVisible(SOLLTEMP + 1);
            else setSollTemperatureVisible(SOLLTEMP + 1 / 1.8)
        })
...
function onHeatClickVolcano() {
    if (!isBleProcesPendingWithoutAlert_Repeat()) {
        ...
        if (isVolcanoOn == false) {
            ...
            characteristicHeaterOnV.writeValue(buffer).then(service => {
                setBleProcessPending(false)
            }).catch(error => {
            ...
            setVisualVolcanoActive(false);
            setBleProcessPending(true);
            characteristicHeaterOffV.writeValue(buffer).then(service => {
                setBleProcessPending(false)
            }).catch(error => {
                setBleProcessPending(false);
                showError(error)
...
function onAirClickVolcano() {
    if (!isBleProcesPendingWithoutAlert_Repeat()) {
        ...
        if (isPumpOnVolcano != true) {
            ...
            setBleProcessPending(true);
            characteristicVolcanoPumpOn.writeValue(buffer).then(service => {
                setBleProcessPending(false)
            }).catch(error => {
                showError(error);
                setBleProcessPending(false)
            ...
            setBleProcessPending(true);
            characteristicIsPumpOffV.writeValue(buffer).then(service => {
                setBleProcessPending(false)
            }).catch(error => {
                setBleProcessPending(false);
                showError(error)
...
function handleCurrTemperatureChangedVolcano(event) {
    let currentTemperature = Math.round(convertBLEtoUint16(event.target.value) / 10);
...
function showCurrentTemperature(temperature) {
    ...
    if (isCelsiusActive() == true) istTempLabel.innerText = Math.round(currentTemperatureVolcano).toString() + "\u00b0C";
    else istTempLabel.innerText = Math.round(convertToFahrenheitFromCelsius(currentTemperatureVolcano)).toString() + "\u00b0F"
}
...
function writeSollTemperatureHybrid(val) {
    if (whichDeviceConnected() == "Hybrid") {
        ...
        if (!isBleProcesPending()) {
            setBleProcessPending(true);
            characteristicWriteTempV.writeValue(buffer).then(service => {
                currentThenValue = 48;
                setBleProcessPending(false);
...
function writeBrightnessVolcano() {
    if (!isBleProcesPendingWithoutAlert_Repeat()) {
        ...
        setBleProcessPending(true);
        characteristicLedBrightnessV.writeValue(buffer).then(service => {
            setBleProcessPending(false)
        }).catch(error => {
            showError(error);
            setBleProcessPending(false)
...
function writeAutoOffCountdownVolcano() {
    if (!isBleProcesPendingWithoutAlert_Repeat()) {
        ...
        setBleProcessPending(true);
        characteristicVolcanoShutOff.writeValue(buffer).then(service => {
            setBleProcessPending(false)
        }).catch(error => {
            setBleProcessPending(false);
            showError(error)
...
function clickOnTempLabelClickVolcano() {
    if (whichDeviceConnected() == "Hybrid")
        if (isCelsiusActive() == true) projectRegister2ChangeBit(65536 + MASK_PRJSTAT2_VOLCANO_FAHRENHEIT_ENA);
        else projectRegister2ChangeBit(MASK_PRJSTAT2_VOLCANO_FAHRENHEIT_ENA)
}

function evalPrj1Volcano(currentVal) {
    if ((currentVal & MASK_PRJSTAT1_VOLCANO_HEIZUNG_ENA) == 0) setVisualVolcanoActive(false);
    else setVisualVolcanoActive(true);
    if ((currentVal & MASK_PRJSTAT1_VOLCANO_PUMPE_FET_ENABLE) == 0) setVisualStatePumpVolcano(false);
...
function evalPrj2Volcano(currentVal) {
    if ((currentVal & MASK_PRJSTAT2_VOLCANO_FAHRENHEIT_ENA) == 0) setCelsiusActive(true);
    else setCelsiusActive(false);
    showCurrentTemperature(currentTemperatureVolcano);
    setSollTemperatureVisible(SOLLTEMP);
    if ((currentVal & MASK_PRJSTAT2_VOLCANO_DISPLAY_ON_COOLING) == 0) displayOnCoolingToggleVolcano.checked = true;
...
function updateHoursLabel() {
    useHoursLabel.innerText = useHoursHeatVolcano + "h " + useMinutesHeatVolcano + "m"
}
...
function handleCurrentAutoOffTimeVolcano(event) {
    ...
    if (autoOffTimeVolcano !== tempVal) {
        ...
        document.getElementById("timerInSec").innerText = Math.round(autoOffTimeVolcano / 60) + " min";
...
function vibrationToggleVolcanoHandler() {
    if (!vibrationToggleVolcano.checked) projectRegister3ChangeBit(65536 + MASK_PRJSTAT3_VOLCANO_VIBRATION);
    else projectRegister3ChangeBit(MASK_PRJSTAT3_VOLCANO_VIBRATION)
}

function displayOnCoolingToggleVolcanoHandler() {
    if (!isBleProcesPendingWithoutAlert_Repeat())
        if (displayOnCoolingToggleVolcano.checked) projectRegister2ChangeBit(MASK_PRJSTAT2_VOLCANO_DISPLAY_ON_COOLING);
        else projectRegister2ChangeBit(65536 + MASK_PRJSTAT2_VOLCANO_DISPLAY_ON_COOLING)
}
...
function projectRegister3ChangeBit(val) {
    if (!isBleProcesPending()) {
        let buffer = convertToUInt32BLE(val);
        setBleProcessPending(true);
        characteristicPrj3V.writeValue(buffer).then(service => {
            setBleProcessPending(false)
        }).catch(error => {
            setBleProcessPending(false);
            showError(error)
...
function setSollTemperatureVisible(temperature) {
    ...
    if (temperature <= MAX_CELSIUS_TEMP && temperature >= MIN_CELSIUS_TEMP) SOLLTEMP = temperature;
    if (isCelsiusActive() == true) document.getElementById("sollTemp").innerText = Math.round(SOLLTEMP).toString() + "\u00b0C";
    else document.getElementById("sollTemp").innerText = convertToFahrenheitFromCelsius(SOLLTEMP).toString() + "\u00b0F"
};