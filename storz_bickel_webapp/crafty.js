...
function startFactoryResetFuncCrafty() {
    if (whichDeviceConnected() == "Crafty")
        if (oldCraftyFirmware == false) {
            currentThenValue = 98;
            setBleProcessPending(true);
            var bufferCode = convertToUInt16BLE(1E3);
            characteristicSicherheitscode.writeValue(bufferCode).then(service => {
                currentThenValue = 99;
                ...
                if (targetTemp > 210) targetTemp = convertFromFahrenheitToCelsius(targetTemp);
                SOLLTEMP = targetTemp;
                ...
                if (oldCraftyFirmware ==
                    false) return characteristicautoOffCountdown.readValue()
            }).then(value => {
                console.log("hi");
                if (oldCraftyFirmware == false) {
    ...
    if (whichDeviceConnected() == "Crafty")
        if (oldCraftyFirmware == false) {
            if (!isBleProcesPendingWithoutAlert_Repeat()) {
                setBleProcessPending(true);
                akkuStatusCharacteristic.readValue().then(value => {
                    ...
                    if (akkuStatusReg & 1536 || systemStatusReg & 512 || prjStatusReg & 8200) {
                        let str = "Please contact STORZ & BICKEL with the following information: \n\n" + "SN   :   " + getSerialNumber() + "\n" + "date : 0x" + numHex(Math.floor(Date.now() / 1E3), 8) + "\n" + "val_1: 0x" + numHex(prjStatusReg, 4) + "\n" + "val_2: 0x" + numHex(prjStatusReg2, 4) + "\n" + "val_3: 0x" + numHex(akkuStatusReg, 4) + "\n" + "val_4: 0x" + numHex(akkuStatusReg2, 4) + "\n" + "val_5: 0x" + numHex(systemStatusReg,
                            4);
                        alert(str)
                    } else {
                        var str = "";
                        if (akkuStatusReg & 16640) str = str + "- Please let the device cool down.\n\n";
                        else if (akkuStatusReg & 3) str = str + "- Please charge the device.\n\n";
                        else if (akkuStatusReg2 & 32768) str = str + "- Please use a different charger or cable.\n\n";
                        if (prjStatusReg2 & 1) str = str + "- Vibration disabled. Activate function if required.\n";
                        if (prjStatusReg2 & 2) str = str + "- LED signalization disabled. Activate function if required.\n";
                        if (prjStatusReg2 & 4096) str = str + "- Automatic shutdown disabled: Bluetooth is active at all time causing faster battery drain.\n";
                        if (prjStatusReg & 32768) str = str + "- Please perform factory reset by pressing the power button for 10s.\n";
                        if (brightnessLED < 10) str = str + "- LED brightness set to low value. Change brightness if required.\n";
...
function disconnectCrafty() {
    if (whichDeviceConnected() == "Crafty") {
        eventsCrafty(false);
        handleProjectRegister.removeEventListener("characteristicvaluechanged", handleProjectRegisterValChanged);
...
function craftyConnect() {
    ...
    bluetoothDevice.gatt.connect().then(server => {
        ...
        return primaryServiceCraftyUuid1.getCharacteristic("00000021-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 4;
        ...
        currentThenValue = 10;
        return primaryServiceCraftyUuid2.getCharacteristic("00000052-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 11;
        ...
        currentThenValue =
            12;
        return primaryServiceCraftyUuid1.getCharacteristic("00000011-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 13;
        charactersiticCurrTemperatureChanged = characteristic;
        return charactersiticCurrTemperatureChanged.startNotifications().then(_ => {
            charactersiticCurrTemperatureChanged.addEventListener("characteristicvaluechanged", handleCurrTemperatureChanged)
        })
    }).then(_ => {
        currentThenValue = 131;
        return charactersiticCurrTemperatureChanged.readValue()
    }).then(value => {
        updateCurrTemperatureMobile(Math.round(convertBLEtoUint16(value) /
            10));
        currentThenValue = 14;
        return primaryServiceCraftyUuid1.getCharacteristic("00000031-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 15;
        ...
        currentThenValue = 17;
        return primaryServiceCraftyUuid2.getCharacteristic("00000032-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 18;
        ...
        return primaryServiceCraftyUuid3.getCharacteristic("000001c3-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 22;
        statusRegister2 = characteristic;
        return statusRegister2.startNotifications().then(_ => {
            statusRegister2.addEventListener("characteristicvaluechanged", handlePrjstatusRegister2)
        })
    }).then(_ => {
        return statusRegister2.readValue()
    }).then(value => {
        ...
        return primaryServiceCraftyUuid3.getCharacteristic("00000023-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue =
            231;
        ...
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid3.getCharacteristic("000001e3-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        ...
        currentThenValue = 24;
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid2.getCharacteristic("00000072-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 25;
        if (oldCraftyFirmware == false) {
        ...
        if (oldCraftyFirmware ==
            false) {
            firmwareBLEVersion = "V" + value.getUint8(0).toString() + "." + value.getUint8(1).toString() + "." + value.getUint8(2).toString();
            firmwareBLEBootloaderLabel.innerText = firmwareBLEVersion
        }
    }).then(service => {
        currentThenValue = 27;
        return primaryServiceCraftyUuid1.getCharacteristic("00000051-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 28;
        ...
        currentThenValue = 30;
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid1.getCharacteristic("00000061-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 31;
        if (oldCraftyFirmware == false) {
        ...
        if (oldCraftyFirmware == false) {
            shutoff.value = convertBLEtoUint16(value);
            shutOffLabel.innerText = convertBLEtoUint16(value) +
                "s"
        }
    }).then(service => {
        currentThenValue = 33;
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid1.getCharacteristic("00000071-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 34;
        if (oldCraftyFirmware == false) {
            characteristicautoOffCurrentValue = characteristic;
            return characteristicautoOffCurrentValue.startNotifications().then(_ => {
                characteristicautoOffCurrentValue.addEventListener("characteristicvaluechanged", handleCurrentAutoValue)
            })
        }
    }).then(_ => {
        ...
        currentThenValue = 35;
        if (oldCraftyFirmware == false) currentAutoOffValue = convertBLEtoUint16(value)
    }).then(service => {
        currentThenValue = 36;
        return primaryServiceCraftyUuid1.getCharacteristic("00000041-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 37;
        characteristicPowerChanged = characteristic;
        return characteristicPowerChanged.startNotifications().then(_ => {
            characteristicPowerChanged.addEventListener("characteristicvaluechanged", handlePowerChanged)
        })
    }).then(_ => {
        ...
        currentThenValue = 39;
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid1.getCharacteristic("00000081-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 40;
        if (oldCraftyFirmware == false) characteristicHeaterOn = characteristic
    }).then(service => {
        currentThenValue = 41;
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid1.getCharacteristic("00000091-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 42;
        if (oldCraftyFirmware == false) characteristicHeaterOff = characteristic
    }).then(service => {
        currentThenValue = 43;
        return primaryServiceCraftyUuid3.getCharacteristic("00000093-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 44;
        handleProjectRegister = characteristic;
        if (oldCraftyFirmware == false) return handleProjectRegister.startNotifications().then(_ => {
            handleProjectRegister.addEventListener("characteristicvaluechanged", handleProjectRegisterValChanged)
        })
    }).then(_ => {
        ...
        if (prjStatusReg & MASK_PRJSTAT_SUPERBOOST_MODE_ENABLED) setSuperBoostModeMobile(true);
        else if (prjStatusReg & MASK_PRJSTAT_BOOST_MODE_ENABLED) setBoostModeMobile(true);
        if (prjStatusReg & 8200) deviceHasErrors = true;
        setShowSollTempMobile(SOLLTEMP)
    }).then(service => {
        currentThenValue = 46;
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid3.getCharacteristic("000001b3-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 47;
        if (oldCraftyFirmware == false) characteristicSicherheitscode = characteristic
    }).then(service => {
        if (oldCraftyFirmware == false) return primaryServiceCraftyUuid3.getCharacteristic("00000083-4c45-4b43-4942-265a524f5453")
    }).then(characteristic => {
        currentThenValue = 48;
        if (oldCraftyFirmware == false) {
        ...
        if (oldCraftyFirmware == false) {
            systemStatusReg = convertBLEtoUint16(value);
            if (systemStatusReg & 640) deviceHasErrors =
                true;
            return primaryServiceCraftyUuid3.getCharacteristic("00000063-4c45-4b43-4942-265a524f5453")
        }
    }).then(characteristic => {
        currentThenValue = 49;
        if (oldCraftyFirmware == false) {
        ...
        if (oldCraftyFirmware == false) {
            ...
            return primaryServiceCraftyUuid3.getCharacteristic("00000073-4c45-4b43-4942-265a524f5453")
        }
    }).then(characteristic => {
        currentThenValue =
            50;
        if (oldCraftyFirmware == false) {
            ...
            return primaryServiceCraftyUuid3.getCharacteristic("000001d3-4c45-4b43-4942-265a524f5453")
        }
    }).then(characteristic => {
        currentThenValue = 51;
        if (oldCraftyFirmware == false) factoryResetCharacteristic = characteristic;
        ...
        if (error.toString().includes("User cancelled")) $("#waitModal").modal("hide");
            ...
            $("#waitModal").modal("hide");
            alert("Bluetooth connection error CRAFTY: please reload and retry.\n" + error.toString() + "\n" + error.stack + "\n" + currentThenValue)
        }
    })
}
...
function craftyEvalPrjStatusReg2() {
    ...
    if ((prjStatusReg2 & MASK_PRJSTAT2_ENABLE_AUTOBLESHUTDOWN) == 0) bluetoothToggle.checked = false;
    else bluetoothToggle.checked = true;
    if ((prjStatusReg2 & MASK_PRJSTAT2_DISABLE_CHARGELED) == 0) indicatorToggle.checked = true;
    else indicatorToggle.checked = false;
    if ((prjStatusReg2 & MASK_PRJSTAT2_DISABLE_VIBRATION) ==
        0) vibrationToggle.checked = true;
    else vibrationToggle.checked = false
}

function writeAutoOffCountdownCrafty() {
    if (!isBleProcesPendingWithoutAlert_Repeat() && whichDeviceConnected() == "Crafty") {
        ...
        characteristicSicherheitscode.writeValue(bufferCode).then(service => {
            ...
            setBleProcessPending(false);
            characteristicautoOffCountdown.writeValue(buffer).then(service => {
...
function findMyCrafty() {
    if (prjStatusReg2 & MASK_PRJSTAT2_SIGNALGEBER_FIND_DEVICE_ENABLE) alert("Find My CRAFTY mode is active. It deactivates in 30 seconds.");
    else projectRegister2ChangeBitC(MASK_PRJSTAT2_SIGNALGEBER_FIND_DEVICE_ENABLE, true)
}

function onHeatClickCrafty() {
    if (oldCraftyFirmware == false && whichDeviceConnected() == "Crafty")
        if (!isBleProcesPendingWithoutAlert_Repeat()) {
            ...
            if (characteristicIsCraftyOn == false) {
                ...
                characteristicHeaterOn.writeValue(buffer).then(service => {
                ...
                showCraftyActive(characteristicIsCraftyOn);
                characteristicHeaterOff.writeValue(buffer).then(service => {
...
function projectRegister2ChangeBitC(bitmask, val) {
    if (!isBleProcesPending()) {
        ...
        if (val == true) statusRegisterValModified = statusRegisterValModified | bitmask;
        else;
        let buffer = convertToUInt16BLE(statusRegisterValModified);
        setBleProcessPending(false);
        statusRegister2.writeValue(buffer).then(service => {
            return statusRegister2.readValue()
        }).then(value => {
            setBleProcessPending(false);
            prjStatusReg2 = convertBLEtoUint16(value)
...
function writeSollTemperatureCrafty(val) {
    if (whichDeviceConnected() == "Crafty") {
        ...
        if (!isBleProcesPending()) {
            setBleProcessPending(true);
            characteristicWriteTemp.writeValue(buffer).then(service => {
                currentThenValue = 148;
                let buffer = convertToUInt16BLE(BOOSTTEMP * 10);
                setTemperatureWillChangeViaBLE_RESET();
                characteristicWriteBoostTemp.writeValue(buffer).then(service => {
                    setBleProcessPending(false);
...
function writeBrightnessCrafty() {
    if (whichDeviceConnected() == "Crafty")
        if (!isBleProcesPendingWithoutAlert_Repeat()) {
            ...
            setBleProcessPending(true);
            characteristicledBrightness.writeValue(buffer).then(service => {
                setBleProcessPending(false)
            }).catch(error => {
                showError(error);
                setBleProcessPending(false)
...
function showCraftyActive(ena) {
    if (ena) {
        ...
        if (prjStatusReg2 & MASK_PRJSTAT2_SET_TEMP_REACHED) changeActualTemperatureColor("green");
        else changeActualTemperatureColor("orange")
    } else {
...
function handleProjectRegisterValChanged(event) {
    ...
    setBoostModeMobile(false);
    setSuperBoostModeMobile(false);
    if (currentVal & MASK_PRJSTAT_SUPERBOOST_MODE_ENABLED) setSuperBoostModeMobile(true);
    else if (currentVal & MASK_PRJSTAT_BOOST_MODE_ENABLED) setBoostModeMobile(true);
    setShowSollTempMobile(SOLLTEMP)
}

function handleCurrTemperatureChanged(event) {
    updateCurrTemperatureMobile(Math.round(convertBLEtoUint16(event.target.value) / 10))
}
...
function handleCurrentAutoValue(event) {
    if (oldCraftyFirmware == false) {
        ...
        if (currentAutoOffValue != tempVal) {
            ...
            document.getElementById("timerInSec").innerText = currentAutoOffValue + " sec";
...
function bluetoothToggleHandler() {
    if (!isBleProcesPendingWithoutAlert_Repeat())
        if (bluetoothToggle.checked) projectRegister2ChangeBitC(MASK_PRJSTAT2_ENABLE_AUTOBLESHUTDOWN, true);
        else projectRegister2ChangeBitC(MASK_PRJSTAT2_ENABLE_AUTOBLESHUTDOWN, false)
}

function vibrationToggleHandlerCrafty() {
    if (whichDeviceConnected() == "Crafty")
        if (!isBleProcesPendingWithoutAlert_Repeat())
            if (!vibrationToggle.checked) projectRegister2ChangeBitC(MASK_PRJSTAT2_DISABLE_VIBRATION, true);
            else projectRegister2ChangeBitC(MASK_PRJSTAT2_DISABLE_VIBRATION, false)
}

function indicatorToggleHandler() {
    if (!isBleProcesPendingWithoutAlert_Repeat())
        if (!indicatorToggle.checked) projectRegister2ChangeBitC(MASK_PRJSTAT2_DISABLE_CHARGELED, true);
