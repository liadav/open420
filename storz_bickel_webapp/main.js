...
if (window.navigator.userAgent.includes("iPhone") || window.navigator.userAgent.includes("iPad")) {
    ...
    boostMinus.addEventListener("touchstart",
        onMinusButtonBoostMouseDown);
    boostMinus.addEventListener("touchend", onMinusButtonBoostMouseUp);
    superboostPlus.addEventListener("touchstart", onPlusButtonSuperBoostMouseDown);
    superboostPlus.addEventListener("touchend", onPlusButtonSuperBoostMouseUp);
    superboostMinus.addEventListener("touchstart", onMinusButtonSuperBoostMouseDown);
    ...
    tempPlus.addEventListener("mouseup",
        onPlusButtonMouseUp);
    ...
    superboostPlus.addEventListener("mouseup",
        onPlusButtonSuperBoostMouseUp);
    superboostMinus.addEventListener("mousedown", onMinusButtonSuperBoostMouseDown);
...
function updateTemperatureLabelsMobile() {
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap") {
...
function setUsedTimeOfDevice(useHoursValue, useMinutes) {
    if (useMinutes != 0) useHoursLabel.innerText = useHoursValue + "h " + useMinutes + "min";
...
function onMinusButtonSuperBoostMouseDown() {
    if (myTimerSuperBoostHandler !== 0) window.clearTimeout(myTimerSuperBoostHandler);
    myTimerSuperBoostHandler = window.setTimeout(myTimerSuperBoost, timeInMilliSeconds);
    if (whichDeviceConnected() == "Qvap") {
        if (myTimerSuperBoostHandler !== 0) window.clearTimeout(myTimerSuperBoostHandler);
        myTimerSuperBoostHandler = window.setTimeout(myTimerSuperBoost, timeInMilliSeconds);
        if (isCelsius == true) setShowSuperBoostTempMobile(SUPERBOOSTTEMP - 1);
        else setShowSuperBoostTempMobile(SUPERBOOSTTEMP -
            1 / 1.8);
...
function onMinusButtonBoostMouseUp() {
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap")
        if (timerMinusBoostButtonMouseDown != null) timerMinusBoostButtonMouseDown.clear()
}

function onMinusButtonSuperBoostMouseUp() {
    if (whichDeviceConnected() == "Qvap")
        if (timerMinusSuperBoostButtonMouseDown != null) timerMinusSuperBoostButtonMouseDown.clear()
}
...
function onPlusButtonBoostMouseDown() {
    ...
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap") {
        ...
        timerPlusBoostButtonMouseDown = easingTimeout(1E3, function() {
            if (myTimerBoostHandler !==
                0) window.clearTimeout(myTimerBoostHandler);
            myTimerBoostHandler = window.setTimeout(myTimerBoost, timeInMilliSeconds);
            if (isCelsius == true) setShowBoostTempMobile(BOOSTTEMP + 1);
            else setShowBoostTempMobile(BOOSTTEMP + 1 / 1.8)
        })
...
function onPlusButtonSuperBoostMouseDown() {
    ...
    if (whichDeviceConnected() == "Qvap") {
        ...
        superboostWillChange = true;
        timerPlusSuperBoostButtonMouseDown = easingTimeout(1E3,
            function() {
                if (myTimerSuperBoostHandler !== 0) window.clearTimeout(myTimerSuperBoostHandler);
                myTimerSuperBoostHandler = window.setTimeout(myTimerSuperBoost, timeInMilliSeconds);
                if (isCelsius == true) setShowSuperBoostTempMobile(SUPERBOOSTTEMP + 1);
...
function onPlusButtonBoostMouseUp() {
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap")
        if (timerPlusBoostButtonMouseDown != null) timerPlusBoostButtonMouseDown.clear()
}

function onPlusButtonSuperBoostMouseUp() {
    if (whichDeviceConnected() == "Qvap")
        if (timerPlusSuperBoostButtonMouseDown != null) timerPlusSuperBoostButtonMouseDown.clear()
}
...
function myTimerSuperBoost() {
    writeSuperBoostTemperatureQVap(BOOSTTEMP);
    myTimerSuperBoostHandler = null
}

function updateCurrTemperatureMobile(temperature) {
    CURRENTTEMP = temperature;
    if (isCelsius == true) istTempLabel.innerText = temperature.toString() + "\u00b0C";
    else if (temperature == CURRENTTEMP_default) istTempLabel.innerText = CURRENTTEMP_default.toString() + "\u00b0F";
    else istTempLabel.innerText = convertToFahrenheitFromCelsius(temperature).toString() + "\u00b0F"
}
...
function setShowBoostTempMobile(boostTemperature) {
    ...
    if (isCelsius == true) boostTempLabel.innerText = Math.round(BOOSTTEMP).toString() + "\u00b0C";
    else {
...
function setShowSuperBoostTempMobile(superboostTemperature) {
    ...
    if (whichDeviceConnected() == "Qvap");
    if (isCelsius == true) superboostTempLabel.innerText = Math.round(SUPERBOOSTTEMP).toString() + "\u00b0C";
    else {
...
function myTimerSetBoostSuperBoostMobile() {
    if (myTimerSetBoostSuperBoostHandlerMobile !== 0) window.clearTimeout(myTimerSetBoostSuperBoostHandlerMobile);
    if (window.getComputedStyle(sollTempLabel).visibility === "hidden") sollTempLabel.style.visibility = "visible";
...
function animateBoostSuperBoostMobile() {
    if (whichDeviceConnected() == "Crafty" || whichDeviceConnected() == "Qvap")
        if (superBoostModeActive) myTimerSetBoostSuperBoostHandlerMobile = window.setTimeout(myTimerSetBoostSuperBoostMobile, 500);
        else if (boostModeActive) myTimerSetBoostSuperBoostHandlerMobile = window.setTimeout(myTimerSetBoostSuperBoostMobile, 1E3);
...
function setShowSollTempMobile(temperature) {
    ...
    if (whichDeviceConnected() == "Qvap");
    else if (superBoostModeActive) tempSOLLTEMP = limitValueToRange(SOLLTEMP + BOOSTTEMP + superBoostVal, 40, 210);
    else if (boostModeActive) tempSOLLTEMP = limitValueToRange(SOLLTEMP + BOOSTTEMP, 40, 210);
    if (isCelsius == true) sollTempLabel.innerText = Math.round(tempSOLLTEMP).toString() + "\u00b0C";
    else sollTempLabel.innerText = convertToFahrenheitFromCelsius(tempSOLLTEMP).toString() +
        "\u00b0F";
    setShowBoostTempMobile(BOOSTTEMP)
}
...
function setBleProcessPending(status) {
    if (status === true) myTimerBLEpendingHandler = window.setTimeout(myTimerBLEpendingTimeoutFunc, 2E3);
    else if (myTimerBLEpendingHandler !== 0) window.clearTimeout(myTimerBLEpendingHandler);
    bleInProcess = status
}

function isBleProcesPending() {
...
function isBleProcesPendingWithoutAlert() {
    if (bleInProcess == true) return true;
    else return false
}

function isBleProcesPendingWithoutAlert_Repeat() {
    if (bleInProcess == true) {
...
function changeActualTemperatureColor(col) {
    ...
    if (whichDeviceConnected() == "Qvap") label = sollTempLabel;
    if (col == "green") {
...
function onButtonClick() {
    ...
    if (navigator.bluetooth) {
        ...
        if (pieces != null) {
            ...
            if (pieces.length > 0 && pieces[0] < 79) alert("Chrome version outdated: please update your browser.")
        }
        let filters = [];
        ...
        console.log("window.navigator " + result);
        if (window.navigator.userAgent.includes("iPhone") || window.navigator.userAgent.includes("iPad")) {
            ...
            filters.push({
                services: [serviceUuidVolcano1, serviceUuidVolcano2, serviceUuidVolcano3, serviceUuidVolcano4, serviceUuidVolcano5]
            });
            filters.push({
        ...
        try {
            navigator.bluetooth.requestDevice(options).then(device => {
                ...
                if (whichDeviceConnected() == "Hybrid") volcanoConnect();
                else if (whichDeviceConnected() == "Qvap") {
                    if (!isConnectBrowser()) setSerialNumber(bluetoothDevice.name.split(" ")[1]);
                ...
                else alert("Bluetooth connection error GENERAL: please reload and retry.\n" + error.toString() + "\n" + error.stack + "\n" + currentThenValue)
            })
        } catch (error) {
            showError(error)
        }
    } else {
        if (iOS()) alert("Web Bluetooth is not supported by Safari. Please use Bluefy or Web BLE browser.");
        else alert("Web Bluetooth is not supported by this browser. Please use another browser.");
        return 0
    }
}
...
function whichDeviceConnected() {
    ...
    if (typeof bluetoothDevice.name != "undefined" && bluetoothDevice.name.includes("S&B VOLCANO") || typeof bluetoothDevice.deviceName != "undefined" && bluetoothDevice.deviceName.includes("S&B VOLCANO")) return "Hybrid";
    else if (typeof bluetoothDevice.name != "undefined" && bluetoothDevice.name.includes("S&B VY") || typeof bluetoothDevice.deviceName != "undefined" && bluetoothDevice.deviceName.includes("S&B VY")) return "Qvap";
    else return "Crafty"
}
...
function convertBLEtoUint24(bleBuf) {
    return bleBuf.getUint8(0) + bleBuf.getUint8(1) * 256 + bleBuf.getUint8(2) * 256 * 256
}

function convertBLEtoUint32(bleBuf) {
    return bleBuf.getUint8(0) + bleBuf.getUint8(1) * 256 + bleBuf.getUint8(2) * 256 * 256 + bleBuf.getUint8(3) * 256 * 256 * 256
}
...
function hexToBytes(hex) {
    let bytes = [];
    for (let c = 0; c < hex.length; c = c + 2) bytes.push(parseInt(hex.substr(c, 2), 16));
    return bytes
}
...
function preventScreenDimming(state) {
    if (typeof bluetooth != "undefined")
        if (typeof bluetooth.setScreenDimEnabled === "function") bluetooth.setScreenDimEnabled(state)
}

function generateErrorMsg(errMsg, verbose) {
    let postStr = "serialNumber=" + getSerialNumber() + "&error=" + currentThenValue + "&textMsg=" + errMsg;
    fetch("error", {
        method: "post",
